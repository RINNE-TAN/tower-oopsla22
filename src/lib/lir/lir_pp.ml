...
let rec show_typ : typ -> string = function
  ...
  | Tuint -> "uint"
  | Tprod ts ->
      List.map ~f:show_typ ts |> String.concat ~sep:", " |> fun s ->
      "(" ^ s ^ ")"
...
let rec show_value : value -> string = function
  ...
  | Vprod vs ->
      List.map ~f:show_value vs |> String.concat ~sep:", " |> fun s ->
      "(" ^ s ^ ")"

let show_uop : uop -> string = function Ulnot -> "lnot"
...
let rec show_stmt : stmt -> string list = function
  ...
  | Sif (x, ss) ->
      [ "if " ^ Symbol.name x ^ " {" ]
      @ indent (List.concat_map ~f:show_stmt ss)
      @ [ "}" ]
...
let show_module (m : modul) =
  ...
  let body =
    String.concat ~sep:"\n" (indent (List.concat_map ~f:show_stmt m.body))
  in
...
let show_stats (m : modul) =
  Format.sprintf "module '%s' stats\nstatements: %d" (Symbol.name m.name)
