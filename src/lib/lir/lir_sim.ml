...
module State = struct
  type t = { reg : Lir.value IdTable.t; mem : Lir.value array; heap : int ref }
...
let try_dealloc state x p t =
  ...
  if not (Lir.equal_value v (Lir_lower.default t)) then
    raise
      (Errors.RuntimeError
         ( Format.sprintf
             "Could not deallocate variable '%s' using expression '%s'; \
              expected default value, heap contains '%s'"
             (Symbol.name x)
...
let check_regs_cleared (state : State.t) out_arg =
  ...
  assert (Set.is_empty @@ Set.remove remaining out_arg)
...
let rec exec_stmt (state : State.t) s =
  ...
  | Sunassign (_, x, e) ->
      ...
      if not (Lir.equal_value v v') then
        raise
          (Errors.RuntimeError
             ( Format.sprintf
                 "Could not uncompute variable '%s' using expression '%s'; \
                  expected value '%s', got value '%s'"
                 (Symbol.name x) (Lir_pp.show_exp e) (Lir_pp.show_value v)
                 (Lir_pp.show_value v'),
               fun () -> State.debug state ));
      State.del_reg state x
  | Sswap (x1, x2) ->
...
let check_leaks (state : State.t) mem_size =
  ...
  let to_set m =
    m
    |> Array.filter_map ~f:(function Lir.Vnum i -> Some i | _ -> None)
    |> Int.Set.of_array
  in
