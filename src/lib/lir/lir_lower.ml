...
let lower_exp ctx x = function
  ...
  | Eproj (y, i) ->
      let p, t =
        ...
        | Ast.Tprod ts ->
            (List.map ~f:lower_typ ts, lower_typ @@ List.nth_exn ts i)
        | _ -> assert false
      in
...
let rec assigned (add, rem) = function
  | Lir.Sassign (data, key, _) ->
      if Map.mem rem key then (add, Map.remove rem key)
      else (Map.add_exn add ~key ~data, rem)
  | Sunassign (data, key, _) ->
      if Map.mem add key then (Map.remove add key, rem)
      else (add, Map.add_exn rem ~key ~data)
...
let rec lower_stmt ctx = function
  ...
  | Sif (x, s1, s2) ->
      ...
      let add, rem =
        List.fold ~init:(IdMap.empty, IdMap.empty) ~f:assigned s1
      in
      let s1 =
        List.concat_map
          ~f:(replace_assign (Map.key_set add) (Map.key_set rem))
          s1
      in
      let s2 =
        List.concat_map
      ...
      let assign_added =
        Map.to_alist add
        |> List.map ~f:(fun (v, t) -> Lir.Sassign (t, v, Eval (default t)))
      in
