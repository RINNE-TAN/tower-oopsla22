...
module Context = struct
  ...
  let add_type ctx key data =
    match Map.add ctx.type_aliases ~key ~data with
    | `Ok ctx -> Some ctx
    | _ -> None

  let get_type ctx key = Map.find ctx.type_aliases key
end
...
let rec equiv_type t1 t2 =
  ...
  | Tprod ts1, Tprod ts2 -> (
      match List.for_all2 ts1 ts2 ~f:equiv_type with Ok v -> v | _ -> false)
  | Tptr None, Tptr _ | Tptr _, Tptr None -> true
  | Tptr (Some t1), Tptr (Some t2) when equiv_type t1 t2 -> true
  | _ -> false

let type_norm_equiv ctx t1 t2 =
  equiv_type (normalize_type ctx t1) (normalize_type ctx t2)

let synth_id (ctx : Context.t) i =
  match Map.find ctx.locals i with
  | Some t -> t
  | None -> (
      match Map.find ctx.statics i with
      | Some t -> t
...
let check_dups args f =
  (List.fold args ~init:IdSet.empty ~f:(fun acc -> function
     | Vvar v -> if Set.mem acc v then Errors.dup_arg f v else Set.add acc v
     | _ -> acc)
    : IdSet.t)
  |> ignore
...
let check_args_match ctx args (arg_types : (id * typ) list) f =
  match
    List.iter2 args arg_types ~f:(fun t (name, t') ->
        if not (type_norm_equiv ctx t t') then
          Errors.argument_type_mismatch f name t' t)
  with
  | Ok _ -> ()
  | _ ->
      Errors.argument_list_length f (List.length args) (List.length arg_types)
...
let rec synth_exp' (ctx : Context.t) = function
  ...
  | Ebop (bop, v1, v2) -> (
      ...
      | (Band | Bor | Beq | Bneq | Bless | Bgreater | Ble | Bge), Tbool, Tbool
        ->
          Tbool
  ...
  | Efun { name; bound; args } -> (
      match Map.find ctx.funcs name with
      ...
      | Some { is_bounded; arg_types; result_type } ->
          ...
          | true, Some (Ovar b | Ominus (b, _)), Some b' when equal_id b b' ->
              ()
          | true, Some (Ovar b | Ominus (b, _)), _ ->
              Errors.unknown_identifier b
...
let rec check_pat forward (ctx : Context.t) pat t =
  ...
  | Pid key, data -> (
      if forward then
        match Map.add ctx.locals ~key ~data with
        | `Ok locals -> { ctx with locals }
        | `Duplicate -> Errors.identifier_already_bound key
      else
        match Map.find ctx.locals key with
        | Some t ->
            if type_norm_equiv ctx t data then
              { ctx with locals = Map.remove ctx.locals key }
...
let rec check_stmt (ctx : Context.t) = function
  ...
  | Sswap (x1, x2) -> (
      match (Map.find ctx.locals x1, Map.find ctx.locals x2) with
      | Some t1, Some t2 ->
          if type_norm_equiv ctx t1 t2 then ctx
          else Errors.swap_type x1 t1 x2 t2
      | None, _ -> Errors.unknown_identifier x1
      | _, None -> Errors.unknown_identifier x2)
  | Smem_swap (e1, x2) -> (
      match (synth_exp ctx e1, Map.find ctx.locals x2) with
      | t1, Some t2 -> (
          match normalize_type ctx t1 with
          | Tptr None -> ctx
          | Tptr (Some t1') ->
              if type_norm_equiv ctx t1' t2 then ctx
              else Errors.mem_swap_type e1 t1' x2 t2
          | _ -> Errors.mem_swap_not_ptr e1 t1)
      | _, None -> Errors.unknown_identifier x2)
  | Sif (x, s1, s2) -> (
      match Option.map ~f:(normalize_type ctx) (Map.find ctx.locals x) with
      | Some Tbool -> (
          if modifies x s1 || modifies x s2 then Errors.modified_condition x
          else
            let ctx1, ctx2 = (check_stmt ctx s1, check_stmt ctx s2) in
            let diff =
              Map.symmetric_diff ~data_equal:(type_norm_equiv ctx) ctx1.locals
                ctx2.locals
            in
...
let rec check_bound (f : Ast.func) = function
  | Sassign (_, Efun call) | Sunassign (_, Efun call) ->
      if equal_id f.name call.name && not (bound_shrunk f.bound call.bound) then
        Errors.incorrect_bound f.name call.bound
...
let check_func (ctx : Context.t) (f : Ast.func) =
  let locals =
    List.fold f.args ~init:ctx.locals ~f:(fun locals arg ->
        match Map.add locals ~key:arg.name ~data:arg.typ with
        | `Ok m -> m
  ...
  if not (type_norm_equiv ctx t result_type) then
    Errors.incorrect_return_type f.name t result_type;
  let old_locals = Map.key_set locals in
  let new_locals = Map.key_set ctx.locals in
  let added = Set.diff new_locals old_locals in
  let returned v =
    match v with Vvar i -> IdSet.singleton i | _ -> IdSet.empty
  in
  Set.diff added (returned result_value)
  |> Set.choose
  |> Option.iter ~f:Errors.local_not_unbound;
  let removed = Set.diff old_locals new_locals in
  Set.choose removed |> Option.iter ~f:Errors.argument_unbound;
...
let rec check_type_decl ?(under_ptr = false) ctx i = function
  | Tvar v when equal_id v i ->
      if under_ptr then Tvar v else Errors.infinite_type v
  | Tvar v -> (
...
let check_decls ?(init = Context.empty) (decls : Ast.decl list) =
  List.fold decls ~init ~f:(fun ctx decl ->
      ...
      | Dfunc f ->
          ...
          let data : Context.func =
            { is_bounded; arg_types; result_type = snd f.result }
          in
          let funcs =
            match Map.add ctx.funcs ~key:f.name ~data with
            | `Ok m -> m
      ...
      | Dstatic a ->
          Option.iter a.values ~f:(fun vs ->
              if List.length vs = a.size then
                if
                  List.for_all vs ~f:(fun v ->
          ...
          let statics =
            match
              Map.add ctx.statics ~key:a.name ~data:(Tptr (Some a.elt_type))
            with
            | `Ok m -> m
