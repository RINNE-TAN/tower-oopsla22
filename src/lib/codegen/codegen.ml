...
let with_uncomputed g1 g2 =
  Gates.concat [ Gates.concat [ g1; g2 ]; Gates.rev g1 ]
...
let copy ws1 ws2 =
  Gates.list @@ List.map2_exn ~f:(fun w1 w -> Pnot (w, [ w1 ])) ws1 ws2

let land_ ws1 ws2 ws =
  Gates.list
  @@ List.map3_exn ~f:(fun w1 w2 w -> Pnot (w, [ w1; w2 ])) ws1 ws2 ws
...
let add a ws1 ws2 ws =
  ...
  let g =
    ...
    @@ List.mapi wss ~f:(fun i ((w1, w2), (w, a)) ->
           ...
           | 0 ->
               Gates.list
                 [ Pnot (a, [ w1; w2 ]); Pnot (w, [ w1 ]); Pnot (w, [ w2 ]) ]
           | i when i = n - 1 ->
               let a' = List.nth_exn anc (i - 1) in
               Gates.list
                 [ Pnot (w, [ w1 ]); Pnot (w, [ w2 ]); Pnot (w, [ a' ]) ]
           | i ->
               let a' = List.nth_exn anc (i - 1) in
               Gates.list
                 [
...
let mul a ws1 ws2 ws =
  ...
  let g, res =
    List.foldi ws2
      ...
      ~f:(fun i (g, acc) w2 ->
        let shifted =
          List.sub zeroes ~pos:0 ~len:i @ List.sub ws1 ~pos:0 ~len:(n - i)
        in
        let anc = Alloc.alloc_n a n in
...
let binary_op a bop ws1 ws2 ws =
  ...
  | Beq ->
      let w = match ws with [ w ] -> w | _ -> assert false in
      let a1 = Alloc.alloc a in
      let a2 = Alloc.alloc a in
...
let mem_swap ws1 ws2 ~mem:(anc, mem) ~cell_size =
  ...
  let s =
    Gates.concat
    @@ List.map2_exn (List.chunks_of mem ~length:cell_size) anc
         ~f:(fun cell i ->
...
let rec wires = function
  ...
  | Gnum _ | Gbool _ | Gnot _ | Gcopy _ | Gbop _ | Gswap _ | Gmem_swap _ ->
      WireSet.empty

let rec lower_gate a ~mem ~cell_size = function
  | Circuit.Gbool (b, w) ->
      if b then Gates.list [ Pnot (w, []) ] else Gates.list []
  | Gnum (n, ws) -> num n ws
  ...
  | Gcond (c, g) ->
      Gates.concat @@ List.map ~f:(lower_gate a ~mem ~cell_size) g |> cond c
  | Gconj g ->
      Gates.rev @@ Gates.concat @@ List.map ~f:(lower_gate a ~mem ~cell_size) g
  | Ginit _ -> Gates.list []

let lower_module (m : Circuit.modul) ~num_cells =
  let a = Alloc.empty () in
  List.map ~f:wires m.body |> WireSet.union_list
  |> Set.iter ~f:(Alloc.mark_allocated a);
  ...
  let body =
    Gates.concat
      [
        c;
        Gates.concat
        @@ List.map ~f:(lower_gate a ~mem ~cell_size:m.cell_size) m.body;
      ]
  in
  {
    name = m.name;
