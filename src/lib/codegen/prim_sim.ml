...
let sim_gate ctx = function
  | Pnot (w, cs) ->
      if List.for_all ~f:(get ctx) cs then
        let v = get ctx w in
...
let check_leaks state m =
  ...
  let final_mem =
    List.map mem ~f:(fun x ->
        if Hashtbl.find_or_add state ~default:(fun () -> false) x then "1"
        else "0")
    |> List.chunks_of ~length:word_size
    |> List.map ~f:(fun l ->
           String.concat l ~sep:"" |> String.rev |> fun s ->
           "0b" ^ s |> Int.of_string)
  in
  let initial_mem =
  ...
  let final_mem =
    Set.union (Int.Set.of_list final_mem) @@ Int.Set.of_list m.hp
  in
  Int.Set.equal initial_mem final_mem && Set.mem final_mem 0
...
let interp (modules : modul list) =
  ...
  List.iter modules ~f:(fun m ->
      if m.name = Symbol.get_sym "main" then (
        ...
        if not (check_leaks state m) then
          Format.eprintf
