...
module State = struct
  type t = { reg : bool WireTable.t; mem : int array; hp : int array }

  let init ~mem_size =
    {
      reg = WireTable.create ();
      mem =
        Array.init mem_size ~f:(fun i -> if i = mem_size - 1 then 0 else i + 1);
  ...
  let show_reg state =
    ...
    |> List.map ~f:(fun (i, v) ->
           Format.sprintf "%d -> %s" i (Bool.to_string v))
    |> String.concat ?sep:(Some ", ")
    |> fun s -> "{ " ^ s ^ " }"

  let show_mem state =
    Array.to_list state.mem |> List.map ~f:Int.to_string
    |> String.concat ?sep:(Some "; ")
    |> fun s -> "[ " ^ s ^ " ]"

  let debug state =
    Format.eprintf "*** DEBUG ***\nRegister: %s\nMemory: %s\n%!"
      (show_reg state) (show_mem state)
...
let try_dealloc state x ws t =
  ...
  if v <> 0 then
    raise
      (Errors.RuntimeError
         ( Format.sprintf
             "Could not deallocate variable '%s' using expression '%s'; \
              expected default value, heap contains '%x'"
             (Symbol.name x)
...
let check_regs_cleared (state : State.t) out_arg =
  ...
  assert (
    Set.is_empty
    @@ Set.diff (Set.diff remaining out_arg) (Int.Set.of_array state.hp))
...
let rec exec_gate (state : State.t) fwd s =
  ...
  | Gnot ws ->
      List.iter ws ~f:(fun w ->
          State.set_reg state w @@ not @@ State.get_reg state w)
  | Gcopy (ws1, ws2) ->
      if fwd then (
  ...
  | Gmem_swap (ws1, ws2) ->
      ...
      if p = 0 then raise NullDereference
      else
        let v = State.get_mem state p in
        State.set_mem state p (State.get_word state ws2);
        State.set_word state ws2 v
  | Gcond (w, gs) ->
      if State.get_reg state w then List.iter gs ~f:(exec_gate state fwd)
      else ()
...
let check_leaks (state : State.t) mem_size =
  ...
  let final_mem =
    Set.union (Int.Set.of_array state.mem) @@ Int.Set.of_array state.hp
  in
...
let interp ~mem_size (modules : Circuit.modul list) =
  ...
  List.iter modules ~f:(fun m ->
      if m.name = Symbol.get_sym "main" then (
        ...
        if not (check_leaks state mem_size) then
          Format.eprintf
            "Warning: memory may not have been reset to initial state at \
             program termination.\n"
