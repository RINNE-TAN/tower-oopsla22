...
let show_wires ws =
  "[" ^ (List.map ~f:Int.to_string ws |> String.concat ~sep:", ") ^ "]"

let indent = List.map ~f:(fun s -> "  " ^ s)

let rec show_gate = function
  | Circuit.Gnum (n, ws) ->
      [ "set " ^ Int.to_string n ^ " -> " ^ show_wires ws ]
  ...
  | Gbop (bop, ws1, ws2, ws3) ->
      ...
      in
      [
        bop ^ " " ^ show_wires ws1 ^ "; " ^ show_wires ws2 ^ " -> "
        ^ show_wires ws3;
      ]
  | Gswap (ws1, ws2) -> [ "swap " ^ show_wires ws1 ^ " <-> " ^ show_wires ws2 ]
  | Gmem_swap (ws1, ws2) ->
      [ "mem " ^ show_wires ws1 ^ " <-> " ^ show_wires ws2 ]
  | Gcond (w, gs) ->
...
let rec all_qubits = function
  ...
  | Gcond (_, g) | Gconj g ->
      Sequence.of_list g |> Sequence.map ~f:all_qubits
      |> Sequence.fold ~init:Int.Set.empty ~f:Set.union
  | Gnum _ | Gbool _ | Gnot _ | Gcopy _ | Gbop _ | Gswap _ | Gmem_swap _ ->
      Int.Set.empty

let all_qubits gs =
  Sequence.of_list gs |> Sequence.map ~f:all_qubits
  |> Sequence.fold ~init:Int.Set.empty ~f:Set.union
