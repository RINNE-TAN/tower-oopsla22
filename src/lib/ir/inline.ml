...
module Context : sig
  ...
  type t = { statics : Ir.static list; funcs : Ir.func IdMap.t }

  let empty = { statics = []; funcs = IdMap.empty }
  let add_static ctx data = { ctx with statics = ctx.statics @ [ data ] }

  let add_func ctx key data =
    { ctx with funcs = Map.add_exn ctx.funcs ~key ~data }

  let get_func ctx key = Map.find_exn ctx.funcs key
end
...
let rename_id m x = match Map.find m x with Some y -> y | None -> x
...
let rec locals = function
  | Ir.Sseq ss ->
      List.map ~f:locals ss |> List.fold ~init:IdSet.empty ~f:Set.union
  | Sassign (x, _) -> IdSet.singleton x
  | Sunassign (x, _) -> IdSet.singleton x
  | Sswap _ -> IdSet.empty
  | Smem_swap _ -> IdSet.empty
  | Sif (_, s1, s2) -> Set.union (locals s1) (locals s2)
  | Sdebug -> IdSet.empty
...
let rename_func args x (f : Ir.func) : Ir.func =
  let mappings =
    List.fold2_exn
      ...
      args ~init:IdMap.empty
      ~f:(fun acc key data -> Map.add_exn acc ~key ~data)
    |> Map.add_exn ~key:f.result.name ~data:x
  in
  let mappings =
    Set.fold ~init:mappings
      ~f:(fun acc key ->
        match Map.add acc ~key ~data:(Symbol.nonce ()) with
        | `Ok m -> m
        | _ -> acc)
      (locals f.body)
  in
  ...
  let args =
    List.map2_exn f.args args ~f:(fun x name : Ir.id_typ ->
        { name; typ = x.typ })
  in
...
let rec inline_call ctx bound x (c : Ir.call) =
  ...
  let bound =
    ...
    | Some (n, v1), Some (Ovar v2), Some (Ovar v) when Symbol.equal v1 v2 ->
        Some (n, v)
...
and inline_stmt ctx bound = function
  ...
  | Sif (x, s1, s2) ->
      Sif (x, inline_stmt ctx bound s1, inline_stmt ctx bound s2)
  | Sdebug -> Sdebug
