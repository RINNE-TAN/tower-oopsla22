...
module Context = struct
  type t = { locals : Ir.typ IdMap.t; funcs : Ir.func IdMap.t }

  let empty (funcs : Ir.func list) : t =
    {
      locals = IdMap.empty;
      funcs =
        List.fold ~init:IdMap.empty
          ~f:(fun acc f -> Map.add_exn acc ~key:f.name ~data:f)
          funcs;
    }

  let add_local ctx key data =
    { ctx with locals = Map.add_exn ctx.locals ~key ~data }

  let get_local ctx key = Map.find_exn ctx.locals key
  let remove_local ctx key = { ctx with locals = Map.remove ctx.locals key }
end
...
let synth_exp ctx = function
  ...
  | Efun f -> (Map.find_exn ctx.funcs f.name).result.typ
