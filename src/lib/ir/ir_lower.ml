...
let flatten (stmt : Ir.stmt) : Ir.stmt =
  let rec flatten' stmt =
    ...
    | s -> [ s ]
  in
  match flatten' stmt with [ s ] -> s | ss -> Sseq ss
...
let lower_func ctx ({ name; bound; args; result; body } : Ast.func) : Ir.func =
  ...
  let args =
    List.map args ~f:(fun i ->
        { i with typ = Ast.Check.normalize_type ctx i.typ })
  in
  match result with
  ...
  | result ->
      ...
      let body =
        Ir.Sseq [ body; Sassign (out, Eval (lower_value ctx result)) ]
      in
