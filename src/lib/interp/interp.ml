...
module State = struct
  ...
  let check_leaks state =
    ...
    && Array.mem ~equal:Ir.equal_value state.mem Vnull
  ...
  let debug state =
    Format.eprintf
      ...
      (show_mem state) (show_static state)
  ...
  let get_mem state p =
    if p < state.mem_size then state.mem.(p)
    else state.static.(p - state.mem_size)

  let set_mem state p v =
    if p < state.mem_size then state.mem.(p) <- v
  ...
  let alloc state typ =
    ...
    (match v with Vptr (_, v) -> state.heap := v | _ -> raise OutOfMemory);
    Ir.Vptr (typ, p)
...
let eval_exp (state : State.t) e =
  ...
  | Ebop (bop, x1, x2) -> (
      ...
      | Blsl, Vnum n1, Vnum n2 ->
          if n2 >= state.word_size then
            Errors.word_size_error n2 state.word_size (State.dump state);
          Vnum (n1 lsl n2)
...
let try_dealloc state x p typ =
  ...
  if not (Ir.equal_value v expected) then
    Errors.unassign_deallocate_error x (Ealloc typ) expected v
      (State.dump state);
...
let check_regs_cleared (state : State.t) (func : Ir.func) =
  ...
  let rec returned v =
    ...
    | Vprod vs ->
        vs |> List.map ~f:returned |> List.fold ~init:IdSet.empty ~f:Set.union
    | _ -> IdSet.empty
  in
...
let rec invoke (state : State.t) x args (func : Ir.func) forward =
  ...
  let state' =
    { state with reg; frame = (forward, func.name) :: state.frame }
  in
  if !Args.verbose then
    Format.printf "Invoke %s %s\n%!" (Symbol.name func.name)
  ...
  if forward then (
    ...
    State.set_reg state x
      (State.get_reg state' func.result.name |> resolve_value state'))
  else (
    State.set_reg state' func.result.name
      (State.get_reg state x |> resolve_value state');
    State.del_reg state x;
  ...
  List.iter2_exn args func.args ~f:(fun v arg ->
      ...
      | v ->
          if not (Ir.equal_value v v') then
            Errors.call_modified_error arg.name func.name v v'
              (State.dump state))
...
and call_func (state : State.t) x (f : Ir.call) forward =
  ...
  | Some v -> (
      ...
      | Some 0 ->
          State.set_reg state x
            (Ir.Lower.default
               (Ast.Check.normalize_type state.ctx func.result.typ))
      | Some n ->
          let bound_var =
            match func.bound with Some (Ovar v) -> v | _ -> assert false
          in
...
and exec_stmt (state : State.t) s =
  ...
  | Sunassign (x, Ealloc typ) -> (
      match State.get_reg state x with
      | Vptr (_, p) ->
          try_dealloc state x p (Ast.Check.normalize_type state.ctx typ)
      | _ -> assert false)
  | Sunassign (x, e) ->
      let v = State.get_reg state x in
      let v' = eval_exp state e in
      if not (Ir.equal_value v v') then
        Errors.unassign_unequal_error x e v v' (State.dump state);
      State.del_reg state x
  | Sswap (x1, x2) ->
...
let interp ~mem_size ctx decls =
  ...
  List.iter decls ~f:(function
    | Ir.Dfunc f ->
        ...
        if String.equal (Symbol.name f.name) "main" then (
          exec_stmt state f.body;
          ret := Some (State.get_reg state f.result.name))
    | Dtype _ -> ()
    | Dstatic a ->
        State.alloc_static state
          (Ast.Check.normalize_type state.ctx a.elt_type)
          a.values
        |> State.set_reg state a.name);
  if not (State.check_leaks state) then
    Format.eprintf
