...
let gen_ir decls =
  let ctx =
    try Ast.Check.check_decls ~init:Ast.Check.Context.empty decls
    with Errors.StaticError e ->
...
let interp ctx decls =
  ...
  if !Args.print then (
    Option.iter
      ~f:(fun v -> Format.printf "Output: %s\n" (Ir.Pp.show_value v))
      ret;
...
let gen_prim circuit =
  ...
  if !Args.print then
    List.iter prim_circuit ~f:(fun m ->
        print_endline @@ Prim_circuit.Pp.show_modul m);
  if !Args.verbose then
    List.iter prim_circuit ~f:(fun m ->
        print_endline @@ Prim_circuit.Pp.show_stats m)

let compile funcs =
  if !Args.verbose then Format.eprintf "Lowering...\n%!";
  let lir = List.map funcs ~f:(Lir.Lower.lower_func funcs) in
  if !Args.verbose then
    List.iter lir ~f:(fun m -> print_endline @@ Lir.Pp.show_stats m);
  if !Args.print then
    List.iter lir ~f:(fun m -> print_endline @@ Lir.Pp.show_module m);
  if !Args.interp_lir then interp_lir lir;
  if !Args.verbose then Format.eprintf "Generating circuit...\n%!";
  let circuit =
    List.map lir ~f:Circuit.Gen.(lower_module ~word_size:!Args.word_size)
  in
  if !Args.interp_circuit then interp_circuit circuit;
  if !Args.verbose then
    List.iter circuit ~f:(fun m -> print_endline @@ Circuit.Pp.show_stats m);
